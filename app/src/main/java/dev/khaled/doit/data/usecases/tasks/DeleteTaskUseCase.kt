package dev.khaled.doit.data.usecases.tasks

import dev.khaled.doit.data.model.Task
import dev.khaled.doit.data.repo.TaskRepo
import dev.khaled.doit.data.usecases.UseCase
import dev.khaled.doit.util.UiState
import kotlinx.coroutines.suspendCancellableCoroutine
import javax.inject.Inject
import kotlin.coroutines.resume

data class DeleteTaskParams(
    val task: Task
)

class DeleteTaskUseCase @Inject constructor(
    private val taskRepo: TaskRepo
) : UseCase<DeleteTaskParams, UiState<Pair<Task, String>>> {
    override suspend fun invoke(parameters: DeleteTaskParams): UiState<Pair<Task, String>> {
        return suspendCancellableCoroutine { continuation ->
            taskRepo.deleteTask(parameters.task) { result ->
                continuation.resume(result)
            }
        }
    }
} 