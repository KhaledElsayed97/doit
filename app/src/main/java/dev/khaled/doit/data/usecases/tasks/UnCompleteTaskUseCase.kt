...
data class UnCompleteTaskParams(
    val task: Task
)

class UnCompleteTaskUseCase @Inject constructor(
    private val taskRepo: TaskRepo
) : UseCase<UnCompleteTaskParams, UiState<Pair<Task, String>>> {
    override suspend fun invoke(parameters: UnCompleteTaskParams): UiState<Pair<Task, String>> {
        return suspendCancellableCoroutine { continuation ->
            taskRepo.unCompleteTask(parameters.task) { result ->
